...
// allocations from seccommon.CloneMetadata (one for the Metadata slice, one Name
...
func TestStartAndReceiveAllocBudget(t *testing.T) {
	...
	if nullShare != wantNullShare {
		t.Fatalf("null.State alloc share = %v (full=%v, wire=%v), want %v: seccommon.CloneMetadata budget changed",
